import{c as E,d as I}from"./main-VRu5ClId.js";import{r as q}from"./read-DwIdlCHZ.js";const k=(e="")=>{const t=e!=="";return`
    ${t?`
  ...
  `},_=e=>`
      ...
      `,j=e=>{let t=e.media?.url||"../src/assets/images/no_img.png",a=e.media?.alt||"Post Image";return`
            <div class="post-image">
              <img src="${t}" alt="${a}" />
            </div>
      `},B=e=>{let t=e.author.avatar.url||"../src/assets/images/no_img.png",a=e.author.avatar.alt||"Post Image";return`
            <div class="post-image">
              <img src="${t}" alt="${a}" />
            </div>
      `},U=e=>{const t=e.comments,a=(e.reactions||[]).map(r=>({emoji:r?.symbol||"❓",count:r?.count||0,reactors:r?.reactors||[]}));let i="",c="";if(a.length===0?i='<button><i class="fa-regular fa-heart"></i></button>':c=a.map(r=>`<span class="reaction-item">${r.count} ${r.emoji}</span>`).join(" "),e||t)return`
        <div class="like-comments-wrapper">
          ...
          <div class="count-and-time">
            <div class="posted-time">${L(e.created)}</div>
          </div>
        </div>
      `},W=e=>`
                <p class="post-text">
      ...
      <div class="comment-body">
        ...
        <div class="comment-text-wrapper">
          ...
          <div class="comment-action">
            <p class="posted-time">${L(e.created)}</p>
  ...
  `},z=(e,t)=>{const a=[],i=c=>{t.forEach(r=>{r.replyToId===c&&(a.push(r),i(r.id))})};return i(e),a},J=e=>{const t=e.comments||[];return t.filter(c=>c.replyToId===null).map(c=>M(c,t)).join("")},x=(e,t)=>{const a=r=>{const y=t.find(C=>C.id===r);return y?y.author.name:"unknown"},i=r=>JSON.parse(localStorage.getItem("profile"))?.name===r?'<button class="comment-delete">delete</button>':"";return e.map(r=>`
        <div class="author-name-target reply" id="${r.id}">
          ...
          <div class="reply-body">
            ...
            <div class="reply-action">
              <p class="posted-time">${L(r.created)}</p>
              <button class="like">like</button>
              <button class="reply-to">reply</button>
              <p class="likes">5 likes</p>
            </div>
      ...
      `).join("")},L=e=>{const t=new Date(e),i=new Date-t,c=Math.floor(i/(1e3*60)),r=Math.floor(c/60);return c<1?"just now":c<60?`${c}m `:r<24?`${r}h `:`${Math.floor(r/24)}d`},V=e=>{const t=document.createElement("div");return t.innerHTML=`
  ...
  `,t.firstElementChild};function Z(e){return{id:e.id,title:e.title,body:e.body,created:e.created,updated:e.updated,author:{name:e.author?.name??"Unknown",email:e.author?.email??"",bio:e.author?.bio??"",avatar:e.author?.avatar?.url??"",banner:e.author?.banner?.url??""},media:{url:e.media?.url??"",alt:e.media?.alt??""},tags:Array.isArray(e.tags)?e.tags:[],reactions:Array.isArray(e.reactions)?e.reactions:[],reactionCount:e._count?.reactions??0,commentCount:e._count?.comments??0,comments:Array.isArray(e.comments)?e.comments.map(t=>({id:t.id,body:t.body,created:t.created,replyToId:t.replyToId??null,postId:e.id,author:{name:t.author?.name??"Anonymous",email:t.author?.email??"",bio:t.author?.bio??"",avatar:{url:t.author?.avatar?.url??"",alt:t.author?.avatar?.alt??""}},banner:{url:t.author?.banner?.url??"",alt:t.author?.banner?.alt??""}})):[]}}async function K(e){try{if(!(await fetch(`${E}/${e}`,{method:"DELETE",headers:{Authorization:`Bearer ${localStorage.getItem("token")}`,"X-Noroff-API-Key":`${I}`}})).ok)throw new Error("Post deletion failed")}catch(t){console.error("Error deleting post:",t)}}async function X(e,t){try{if(!(await fetch(`${E}/${e}/comment/${t}`,{method:"DELETE",headers:{Authorization:`Bearer ${localStorage.getItem("token")}`,"X-Noroff-API-Key":`${I}`}})).ok)throw new Error("Comment deletion failed")}catch(a){console.error("Error deleting comment:",a)}}async function F(e,t){try{const a=await fetch(`${E}/${e}/react/${t}`,{method:"PUT",headers:{Authorization:`Bearer ${localStorage.getItem("token")}`,"X-Noroff-API-Key":`${I}`}});if(!a.ok)throw new Error("Like failed");console.log("Like successful",a)}catch(a){console.error(a)}}async function P(e,t,a=null){try{const i={body:t};a&&(i.replyToId=Number(a));const c=await fetch(`${E}/${e}/comment`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${localStorage.getItem("token")}`,"X-Noroff-API-Key":`${I}`},body:JSON.stringify(i)});if(!c.ok)throw new Error("Comment submission failed");return await c.json()}catch(i){console.error("Error submitting comment:",i)}}const Y=()=>{const e=()=>{const l=document.querySelectorAll(".profile-wrapper"),o=()=>{const n=localStorage.getItem("profile"),d=JSON.parse(n).name,m=new URLSearchParams(window.location.search).get("username");return d===m?`
                  <button class="profile-edit">⋯</button>
                  <div class="profile-edit-wrapper">
                    <button class="edit">edit</button>
                    <button class="delete">delete</button>
                  </div>
                  `:""};l.forEach(n=>{const s=n.querySelector(".profile-edit");s.innerHTML=`${o()}`;const d=n.querySelector(".profile-edit-wrapper");s.addEventListener("click",u=>{u.preventDefault(),d.style.display==="flex"?d.style.display="none":d.style.display="flex"});const p=n.querySelector(".edit"),m=n.querySelector(".delete");p&&p.addEventListener("click",u=>{u.preventDefault();const f=n.closest(".post").id;window.location.href=`../posts/update.html?id=${f}`}),m&&m.addEventListener("click",u=>{u.preventDefault();const f=n.closest(".post").id;confirm("Delete this post?")&&(K(f),window.location.href="../posts/feed.html")})})},t=l=>{const o=document.getElementById(l);if(!o)return;const n=o.querySelector(".input-wrapper");n&&(n.innerHTML=k(""),i())},a=()=>{document.querySelectorAll(".reply-to").forEach(o=>{o.dataset.bound!=="true"&&(o.addEventListener("click",n=>{n.preventDefault();const s=o.closest(".post").id,d=o.closest(".author-name-target").id,p=o.closest(".author-name-target").querySelector(".username").textContent,m=o.closest(".post").querySelector(".input-wrapper");m.innerHTML=k(p);const u=m.querySelector("#submit-post-comment"),f=m.querySelector(".comment-input");u.dataset.bound="true",R(),u.addEventListener("click",async v=>{v.preventDefault();const h=f.value.trim();if(!h)return;const w=m.querySelector(".display-replying")!==null,g=await P(s,h,w?d:null);if(!g)return;const b=await q(g.data.owner);if(!b)return;const S={id:g.data.id,body:g.data.body,created:g.data.created,replyToId:w?d:null,postId:s,author:{name:b.data.name,email:b.data.email,avatar:{url:b.data.avatar.url||"../src/assets/images/no_img.png",alt:b.data.avatar.alt||"Profile Image"}}};if(w){const $=o.closest(".comment-body").querySelector(".reply-section"),A=[S],O=x(A,A),D=document.createElement("div");D.innerHTML=O.trim(),$.appendChild(D.firstElementChild);const T=m.querySelector(".display-replying");T&&T.remove()}else o.closest(".post").querySelector(".comment-section").appendChild(temp.firstElementChild);Y(),f.value="",t(s),a(),r(),y()})}),o.dataset.bound="true")})},i=()=>{document.querySelectorAll("#submit-post-comment.top-submit").forEach(o=>{o.dataset.bound!=="true"&&(o.addEventListener("click",async n=>{n.preventDefault();const s=o.closest(".post");if(!s)return;const d=s.querySelector(".input-wrapper");if(!d)return;const p=d.querySelector(".comment-input");if(!p)return;const m=p.value.trim();if(!m)return;const u=d.querySelector(".display-replying");u&&u.remove();const f=s.id,v=await P(f,m);if(!v)return;const h=await q(v.data.owner);if(!h)return;const w={id:v.data.id,body:v.data.body,created:v.data.created,author:{name:h.data.name,email:h.data.email,avatar:{url:h.data.avatar.url||"../src/assets/images/no_img.png",alt:h.data.avatar.alt||"Profile Image"}}},g=s.querySelector(".comment-section"),b=M(w,[]),S=document.createElement("div");S.innerHTML=b.trim(),Array.from(S.children).forEach($=>g.appendChild($)),r(),y(),i(),p.value="",t(f)}),o.dataset.bound="true")})},c=()=>{document.querySelectorAll(".reaction-btns").forEach(o=>{o.addEventListener("mouseenter",()=>{const n=o.closest(".like-comments-wrapper").querySelector(".reaction-container");n.style.display="flex",o.querySelectorAll(".reaction-btn").forEach(d=>{d.addEventListener("click",p=>{p.preventDefault();const m=o.closest(".post").id,u=p.target.dataset.reactionType;F(m,u)})})}),o.addEventListener("mouseleave",()=>{const n=o.closest(".like-comments-wrapper").querySelector(".reaction-container");n.style.display="none"})})},r=()=>{document.querySelectorAll(".option-btn").forEach(o=>{o.dataset.bound!=="true"&&(o.addEventListener("click",n=>{n.preventDefault(),document.querySelectorAll(".option-btn-wrapper").forEach(d=>{d.style.display="none"});const s=o.closest(".reply-option")?.querySelector(".option-btn-wrapper")||o.closest(".comment-option")?.querySelector(".option-btn-wrapper");s&&(s.style.display="flex")}),o.dataset.bound="true")})},y=()=>{document.querySelectorAll(".comment-delete").forEach(o=>{o.dataset.bound!=="true"&&(o.addEventListener("click",async n=>{n.preventDefault();const s=o.closest(".author-name-target").id,d=o.closest(".post").id,p=o.closest(".author-name-target");!confirm("Delete this comment?")||!X(d,s)||p.remove()}),o.dataset.bound="true")})},C=()=>{document.querySelectorAll(".username").forEach(o=>{o.addEventListener("click",n=>{n.preventDefault();const s=o.closest(".post").querySelector(".username").textContent;window.location.href=`../profile/user.html?username=${s}`})})},H=()=>{document.querySelectorAll(".post-image").forEach(o=>{o.addEventListener("click",n=>{n.preventDefault();const s=o.closest(".post").id;window.location.href=`../posts/view.html?id=${s}`})})},R=()=>{const l=document.querySelector(".close-reply-display");!l||l.dataset.bound==="true"||(l.addEventListener("click",o=>{o.preventDefault();const n=l.closest(".display-replying");if(n){const s=n.closest(".post").id;t(s)}}),l.dataset.bound="true")},N=()=>{document.addEventListener("click",l=>{const o=l.target.closest(".option-btn"),n=l.target.closest(".option-btn-wrapper");!o&&!n&&document.querySelectorAll(".option-btn-wrapper").forEach(s=>{s.style.display="none"})})};e(),a(),i(),c(),r(),y(),C(),H(),N()};export{Y as a,Z as f,V as p};
